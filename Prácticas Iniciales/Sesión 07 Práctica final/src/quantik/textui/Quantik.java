...
public class Quantik {
	...
	public static void main(String[] args) {
		try {
			String jugada = null; // Inicio de variable jugada
			boolean flag = false;
			int contador = 0;
			...
			try {
				...
				while (!comprobarSiFinalizaPartida() || flag == false) {// Comprobante de que hasta que no termine no
																		// salga
					mostrarTableroEnFormatoTexto();
					jugada = recogerJugada();
					if (jugada.equals("salir")) { // Si la jugada es "salir" entonces se sale del while
						mostrarInterrupcionPartida();
						finalizarPartida();
						break;
					} else if (contador > 0 && jugada.equals("deshacer")) { // Deshacer una jugada
						deshacer.deshacerJugada();
						contador--;
					} else if (contador == 0 && jugada.equals("deshacer")) { // Si no hay jugadas no deshace
						System.out.println("No hay ninguna jugada en tablero");
					} else if (validarFormato(jugada)) {
						if (validarLegalidad(jugada)) {
							realizarJugada(jugada);
							if (comprobarSiFinalizaPartida()) {// Se comprueba si se finaliza la partida antes de
																// cambiar el
																// turno
								mostrarTableroEnFormatoTexto();
								finalizarPartida();
