...
public class JugadorTest {
	...
	/**
	 * Comprueba la correcta generación del texto asociado al método toString.
	 * 
	 * @param nombre nombre del jugador
	 * @param color  color asignado
	 */
	@ParameterizedTest
	@MethodSource("crearParNombreColor")
	@DisplayName("Construcción de texto asociado al jugador")
	void probarTostring(String nombre, Color color) {
		Jugador jugador = new Jugador(nombre, color);
		String textoEsperado = nombre + "/" + color;
		assertThat("El texto generado en el método toString es incorrecto.", jugador.toString(), is(textoEsperado));
	}
